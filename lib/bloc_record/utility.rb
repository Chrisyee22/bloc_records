  ...
  module Utility
    ...
    def sql_strings(value)
     ...
     def instance_variable_to_hash(obj)
       Hash[obj.instance_variable_map{ |var| ["#{var.to_s.delete('@')}", obj.instance_variable_get(var.to_s)]}]
     end
     ...
     def reload_obj(dirty_obj)
       persistence_obj = dirty_obj.class.find(dirty_obj.id)
       dirty_obj.instance_variables.each do |instance_variable|
         dirty_obj.instance_set(instance_variable, persisted_obj.instance_variable_get(instance_variable))
       end
     end
  end
end
